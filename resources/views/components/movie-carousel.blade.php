...
@if(!empty($movies))
    ...
    <div class="relative overflow-hidden py-1.5 sm:py-2 sm:pr-2 touch-pan-x" id="{{ $carouselId }}">
        ...
        <button class="absolute top-1/2 left-1 -translate-y-1/2 w-10 h-10 bg-black/60 hover:bg-black/80 rounded-full flex items-center justify-center z-10 border-none text-white cursor-pointer hidden md:flex"
        ...
        <button class="absolute top-1/2 right-1 -translate-y-1/2 w-10 h-10 bg-black/60 hover:bg-black/80 rounded-full flex items-center justify-center z-10 border-none text-white cursor-pointer hidden md:flex"
                data-carousel="{{ $carouselId }}">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24"
                 stroke="currentColor" stroke-width="2">
